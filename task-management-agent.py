...
def prompt_ai(messages, nested_calls=0):
    ...
    asana_chatbot = ChatGoogleGenerativeAI(model=model)
...
def main():
    ...
    if prompt := st.chat_input("What would you like to do today?"):
        ...
        # Gemini can return content as a list of content blocks (e.g.
        # [{'type': 'text', 'text': '...', 'extras': {...}}]) instead of a
        # plain string. Normalize to plain text before storing it, since
        # AIMessage(content=...) expects a string or well-formed block list.
